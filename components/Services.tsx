'use client';

import React from 'react';
import { Globe, Smartphone, Server, MessageSquare, LineChart } from 'lucide-react';
import { useCursor } from '../contexts/CursorContext';
import { Service } from '../types';

// Custom WhatsApp Icon Component
const WhatsAppIcon = ({ size = 32 }: { size?: number }) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 24 24" 
    fill="currentColor" 
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.72.937 3.659 1.432 5.63 1.433h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
);

const Services: React.FC = () => {
  const { setCursorType } = useCursor();

  const services: Service[] = [
    {
      id: 1,
      title: "Web Applications",
      description: "Scalable, high-performance React & Next.js applications tailored for enterprise growth.",
      icon: <Globe size={32} />
    },
    {
      id: 2,
      title: "Mobile Development",
      description: "Native and cross-platform mobile apps providing seamless experiences on iOS and Android.",
      icon: <Smartphone size={32} />
    },
    {
      id: 3,
      title: "WhatsApp & AI Agents",
      description: "Deploy autonomous AI agents directly to WhatsApp to handle customer queries and sales 24/7.",
      icon: <WhatsAppIcon size={32} />
    },
    {
      id: 4,
      title: "Backend Services",
      description: "Robust API architecture, microservices, and cloud infrastructure management.",
      icon: <Server size={32} />
    },
    {
      id: 5,
      title: "Technical Strategy",
      description: "Strategic consulting to optimize your digital roadmap and AI implementation stack.",
      icon: <LineChart size={32} />
    },
    {
      id: 6,
      title: "Conversational UI",
      description: "Intuitive voice and chat interfaces integrated with WhatsApp Business API.",
      icon: <MessageSquare size={32} />
    }
  ];

  return (
    <section id="services" className="py-32 bg-lumina-dark relative">
      <div className="container mx-auto px-6">
        <div className="mb-20 text-center">
          <h2 className="text-sm font-bold text-lumina-accent uppercase tracking-widest mb-3">Our Expertise</h2>
          <h3 className="text-4xl md:text-5xl font-display font-bold text-white">Engineering Excellence</h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service) => (
            <div
              key={service.id}
              onMouseEnter={() => setCursorType('button')}
              onMouseLeave={() => setCursorType('default')}
              className="p-8 border border-gray-800 bg-gray-900/20 backdrop-blur-sm rounded-2xl hover:border-lumina-accent/50 hover:bg-gray-800/50 transition-all duration-300 group"
            >
              <div className="mb-6 text-gray-400 group-hover:text-lumina-accent transition-colors">
                {service.icon}
              </div>
              <h4 className="text-xl font-bold text-white mb-4 font-display">{service.title}</h4>
              <p className="text-gray-400 leading-relaxed text-sm">{service.description}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default Services;
